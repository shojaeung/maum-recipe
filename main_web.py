...
# 3. 화면 구성
st.title("❤ 마음 메뉴판 ❤")
st.write("마음 속의 감정 재료들을 소개합니다!")

user_input = st.text_input("이 감정, 어떤 재료인지 알고 싶으신가요? 우리의 마음 메뉴판에 검색해 보세요.", placeholder="예: 감동함, 서운함, 홀가분함")
...
if user_input:
    ...
    # --- 3단계: 결과 출력 ---
    if matched_actual_file:
        st.success(f"▶ '{matched_emotion_name}'(이)라는 감정 재료를 발견했어요! 지금 보여줄게요.")
        img = Image.open(matched_actual_file)
        st.image(img, use_container_width=True)
    else:
        st.warning("미안해요, 그 재료는 아직 준비 중이에요.")
